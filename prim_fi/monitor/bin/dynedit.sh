...
menu() {
    ...
    dynchk_copy=" "
    dynchk_copy=`awk -F, -f /Dmon/utils/dmdynchk_copy.awk objname=$2 $1`
    chk_clipboard=""
    ...
    dmmenu_columns="1"
    dmmenu_options="AOL"
#echo /Dmon/utils/dmMenu $dmmenu_title $dmmenu_columns \"$dmmenu_list\" $dmmenu_options >/dev/null
    /Dmon/utils/dmMenu -200 -40 "cC" $dmmenu_title $dmmenu_columns \"$dmmenu_list\" $dmmenu_options
...
yes_no() {
    ...
    dmnotice_arg=""
    /Dmon/utils/dmNotice -200 -40 "cC" $dmnotice_title $dmnotice_buttons $dmnotice_options "Are you sure ?" $dmnotice_args
...
list() {
    ...
    dmnotice_options="AOL "
    dmnotice_arg=""
    /Dmon/utils/dmNotice -200 -40 "cC" $dmnotice_title $dmnotice_buttons $dmnotice_options "$2" $dmnotice_args &
...
case $menu_ret in
     E)
        ...
        then
            TempFileName=$(echo $1|sed 's%/%\\%g')
            Mew32.exe /NI $TempFileName /L$lineno
     ...
     C)
         ans=copy
         rm C_* 2> /dev/null
         awk -F, -f /Dmon/utils/dmdyncopy.awk copy_type="ALL" objname=$2 $1> dyncopy.txt
         ;;
     C_ALLWITHOUTPARL)
     ...
     C_*)
         ans=copy_$2
         awk -F, -f /Dmon/utils/dmdyncopy.awk objname=$2 copy_type=$menu_ret $1 > dyncopy.txt
         ;;
     P)
         ans=paste
         ...
         then
             ...
             if [ "$yesno" = "y" ] || [ "$yesno" = "Y" ]; then
                 awk -F, -f /Dmon/utils/dmdyncopy.awk  copy_type="NOTHING" objname=$2 $1 > dynobject.txt
                 awk -F, -f /Dmon/utils/dmdyndelete.awk objname=$2 $1 > dyndelete.txt
                 awk -F, -f /Dmon/utils/dmdynpaste.awk objname=$2 dyncopy.txt dyndelete.txt > dynpaste.txt
                 cp dynpaste.txt $1
             fi
         else
     ...
     P_ALL_WITHOUT_PARL)
         ...
         if [ "$yesno" = "y" ] || [ "$yesno" = "Y" ]; then
            awk -F, -f /Dmon/utils/dmdyncopy.awk  copy_type="OBJECT" objname=$2 $1 > dynobject.txt
            awk -F, -f /Dmon/utils/dmdyndelete.awk objname=$2 $1 > dyndelete.txt
            ...
            awk -F, -f /Dmon/utils/dmdynpaste.awk objname=$2 dyncopy.txt_tmp dyndelete.txt > dynpaste.txt
            cp dynpaste.txt $1
         fi
         ;;
     P_*)
         ans=paste_$2
         s=`echo $menu_ret | awk -F, -f /Dmon/utils/dmsubstr.awk first=3`
         yes_no "Paste..."$s"->"$2
         if [ "$yesno" = "y" ] || [ "$yesno" = "Y" ]; then
            rm M_???? 2>/dev/null
     ...
     V)
         ans=view
         list "View:"$2 "`awk -F, -f /Dmon/utils/dmdynshow.awk objname=$2 $1`"
         ;;
     CL_ALL)
         ans=clipboard...all
     ...
     CL_*)
         ...
         filename=`echo $menu_ret | awk -F, -f /Dmon/utils/dmsubstr.awk first=4 `
     ...
     U)
         ...
         then
             ...
             if [ "$yesno" = "y" ] || [ "$yesno" = "Y" ]; then
                 awk -F, -f /Dmon/utils/dmdyndelete.awk objname=$2 $1 > dyndelete.txt
                 awk -F, -f /Dmon/utils/dmdynundo.awk objname=$2 dynobject.txt dyndelete.txt > dynundo.txt
                 cp dynundo.txt $1
             fi
         else
